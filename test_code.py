...
from   randomized_A   import randomized
...
if __name__ == "__main__":
    # NEED TO FIGURE OUT WHAT IS TO BE DONE FOR NO INSTANCES
    test_cases = 10
    redundancy = 20
    passed = 0
    enablePrint = False
    ...
    for i in range(test_cases):
        for j in range(redundancy):
            ...
            G_correct         = deepcopy(G)
            vertex_cover_size = random.randint(0, int(i*i/2))
            G_brute_force     = deepcopy(G)
            G_kernel          = deepcopy(G) 
            # G_randomized      = deepcopy(G)
            start = time.time()
            ...
            kernel_A_time = end 
            # start = time.time()
            # vertex_cover_rA   = randomized(G_kernel, vertex_cover_size)
            # end = time.time() - start 
            # random_A_time = end 
            try:
            ...
            # try:
            #     time_randomized_A[i] += (random_A_time)
            # except:
            #     time_randomized_A.append(random_A_time)
            if vertex_cover_bf and ((len(vertex_cover_bf[0]) > vertex_cover_size) or len(vertex_cover_kA[0]) != len(vertex_cover_bf[0])):
                ...
                print("vertex_cover_rA: ", vertex_cover_rA)
                draw_graph(G)
                continue
            if enablePrint:
    ...
    # plt.plot([i for i in range(test_cases)], time_randomized_A)
